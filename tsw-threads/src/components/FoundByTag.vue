...
async function getPosts(){
    const fetch = axios.get(`/api/threads/find/${tag.value}/${lastPage.value}/${30}`,{withCredentials:true}
    ).then((res)=>{
...
async function hidePost(pid){
  const fetch = axios.post(`/api/threads/post/hide/${pid}`,{},{withCredentials:true}).catch((err)=>{
    console.log(err)
  })
}
  ...
  <div class="container">
    <ul class="thread-list">
      <li v-for="post in posts" :key="post._id" class="thread-item">
        ...
        <div class="actions">
          <button v-if="!me.isBlockedEverywhere && (me.isAdmin || me?._id?.toString?.() === post?.creatorId?.toString?.())" class="btn delete" @click="hidePost(post._id)">Hide</button>
